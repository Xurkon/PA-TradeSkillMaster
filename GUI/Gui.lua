...
-- MODERNIZED: Enhanced button with better hover effects and modern styling
function TSMAPI.GUI:CreateButton(parent, textHeight, name, isSecure)
	...
	-- Modern highlight effect (brighter, more noticeable)
	local highlight = btn:CreateTexture(nil, "HIGHLIGHT")
	highlight:SetAllPoints()
	highlight:SetTexture(1, 1, 1, .15)  -- Slightly less bright but still visible
	highlight:SetBlendMode("ADD")  -- ADD blend for better modern glow effect
	btn.highlight = highlight
	
	-- Add subtle hover glow animation
	local hoverGlow = btn:CreateTexture(nil, "OVERLAY")
	hoverGlow:SetAllPoints()
	hoverGlow:SetTexture("Interface\\Buttons\\WHITE8X8")
	hoverGlow:SetVertexColor(100/255, 180/255, 255/255, 0)  -- Blue glow, initially invisible
	hoverGlow:SetBlendMode("ADD")
	btn.hoverGlow = hoverGlow
	
	-- Enhanced hover behavior with modern blue glow
	btn:SetScript("OnEnter", function(self)
		...
		fadeIn = function()
			...
			self.hoverGlow:SetAlpha(alpha)
			if alpha < 0.2 then
				C_Timer.After(0.01, fadeIn)
			end
		end
		fadeIn()
		
		if self.tooltip then ShowTooltip(self) end
	end)
	
	btn:SetScript("OnLeave", function(self)
		-- Animate glow out
		local alpha = self.hoverGlow:GetAlpha()
		local fadeOut
		fadeOut = function()
			alpha = max(alpha - 0.04, 0)
			self.hoverGlow:SetAlpha(alpha)
			if alpha > 0 then
				C_Timer.After(0.01, fadeOut)
			end
		end
		fadeOut()
