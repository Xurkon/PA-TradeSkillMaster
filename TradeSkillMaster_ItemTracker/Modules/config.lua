...
local function GetSTData()
	...
	local function AddItem(itemString, key, quantity)
		items[itemString] = items[itemString] or
		{ total = 0, bags = 0, bank = 0, guild = 0, auctions = 0, mail = 0, personalBank = 0, realmBank = 0 }
	...
	for name, selected in pairs(filters.characters) do
		if selected then
			...
			for itemString, quantity in pairs(TSM:GetPlayerPersonalBank(name) or {}) do
				AddItem(itemString, "personalBank", quantity)
			end
			-- Ascension: Realm Bank
			for itemString, quantity in pairs(TSM:GetPlayerRealmBank(name) or {}) do
				AddItem(itemString, "realmBank", quantity)
			end
		end
	end
		...
		if (not name or filters.name == "" or strfind(strlower(name), filters.name)) and (not filters.group or groupPath and strfind(groupPath, "^" .. TSMAPI:StrEscape(filters.group))) then
			tinsert(rowData, {
				cols = {
					...
					{
						value = data.personalBank, -- Ascension
						sortArg = data.personalBank,
					},
					{
						value = data.realmBank, -- Ascension
						sortArg = data.realmBank,
					},
					{
						value = data.total,
						sortArg = data.total,
...
function Config:LoadInventoryViewer(container)
	...
	if not viewerST then
		local stCols = {
			...
			{
				name = L["PBank"], -- Ascension: Personal Bank
				width = 0.07,
			},
			{
				name = L["RBank"], -- Ascension: Realm Bank
				width = 0.07,
			},
			{
				name = L["Total"],
					...
					children = {
						{
							...
							tooltip = L
							["If you rename / transfer / delete one of your characters, use this dropdown to remove that character from ItemTracker. There is no confirmation. If you accidentally delete a character that still exists, simply log onto that character to re-add it to ItemTracker."],
						},
						{
							type = "Dropdown",
							...
							tooltip = L
							["Select guilds to ingore in ItemTracker. Inventory will still be tracked but not displayed or taken into consideration by Itemtracker."],
						},
						{
							type = "Dropdown",
							...
							tooltip = L
							["Specifies the market value price source used for \"Total Market Value\" in the Inventory Viewer."],
						},
					},
				},
			},
			...
			children = {
				{
					...
					tooltip = L
					["Here, you can choose what ItemTracker info, if any, to show in tooltips. \"Simple\" will only show totals for bags/banks and for guild banks. \"Full\" will show detailed information for every character and guild."],
				},
			},
		},
	}
