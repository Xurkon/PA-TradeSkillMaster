...
local characterDefaults = {
	...
	bags = {},
	bank = {},
	personalBank = {}, -- Ascension Personal Bank
	realmBank = {}, -- Ascension Realm Bank
	auctions = {},
	guild = nil,
	mail = {},
	mailInbox = {},
	lastUpdate = { bags = 0, bank = 0, auctions = 0, mail = 0, guild = 0, personalBank = 0, realmBank = 0 },
	account = nil,
}
...
function TSM:RegisterModule()
	...
	TSM.moduleAPIs = {
		...
		{ key = "personaltotal",      callback = "GetPersonalBankTotal" }, -- Ascension
		{ key = "realmtotal",         callback = "GetRealmBankTotal" }, -- Ascension
	}
...
function TSM:GetTooltip(itemString)
	...
	if TSM.db.global.tooltip == "simple" then
		...
		if grandTotal > 0 then
			tinsert(text,
				{
					left = "  " .. "ItemTracker:",
					right = format(L["(%s player, %s alts, %s guild banks, %s AH)"],
	...
	if #text > 0 then
		tinsert(text, 1,
			{
				left = "|cffffff00" .. "TSM ItemTracker:",
				right = format(L["%s item(s) total"],
					"|cffffffff" .. grandTotal .. "|r")
			})
	end
...
function TSM:GetPlayerTotal(itemString)
	...
	for name, data in pairs(TSM.characters) do
		if name == TSM.CURRENT_PLAYER then
			...
			playerTotal = playerTotal + (data.personalBank and data.personalBank[itemString] or 0) -- Ascension
			playerTotal = playerTotal + (data.realmBank and data.realmBank[itemString] or 0) -- Ascension
		else
			...
			altTotal = altTotal + (data.personalBank and data.personalBank[itemString] or 0) -- Ascension
			altTotal = altTotal + (data.realmBank and data.realmBank[itemString] or 0) -- Ascension
		end
	end
