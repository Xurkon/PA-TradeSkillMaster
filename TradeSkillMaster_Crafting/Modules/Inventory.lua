...
function Inventory:GetTotals()
	...
	for _, player in pairs(TSMAPI:ModuleAPI("ItemTracker", "playerlist") or {}) do
		if player == UnitName("player") or not TSM.db.global.ignoreCharacters[player] then
			local personalBank = TSMAPI:ModuleAPI("ItemTracker", "playerpersonalbank", player) or {}
	...
	local realmBank = TSMAPI:ModuleAPI("ItemTracker", "playerrealmbank") or {}
...
function Inventory:GetItemSources(crafter, neededMats)
	...
	local crafterPersonalBank = TSMAPI:ModuleAPI("ItemTracker", "playerpersonalbank", crafter) or {}
	local totalQtyCache = {}
	...
	for itemString, quantity in pairs(neededMats) do
		if TSMAPI:GetVendorCost(itemString) then
			local vendorNeed = quantity -
			((crafterBags[itemString] or 0) + (crafterMail[itemString] or 0) + (crafterBank[itemString] or 0) + (crafterPersonalBank[itemString] or 0))
		...
		elseif TSMAPI.Conversions[itemString] and TSMAPI.InkConversions[itemString] then
			...
			if data.source == "vendortrade" then
				...
				if quantity > itemQty and num >= (quantity - itemQty) then
					...
					neededMats[tradeItem] = (neededMats[tradeItem] or 0) +
					quantity / data.rate                                          -- add the qty of IOD to needed mats
				end
			end
		end
	end
	...
	for _, player in pairs(TSMAPI:ModuleAPI("ItemTracker", "playerlist") or {}) do
		if player == crafter or not TSM.db.global.ignoreCharacters[player] then
			...
			for itemString in pairs(neededMats) do
				...
				-- Ascension WoW: check personal bank for items
			...
			-- Ascension WoW: Visit Personal Bank task
			if next(personalBankItems) then
				tinsert(task, { taskType = L["Visit Personal Bank"] or "Visit Personal Bank", items = personalBankItems })
			end
			...
			if next(task) then
				tinsert(sources,
					{ sourceName = player, isCrafter = player == crafter, isVendor = false, isAH = false, tasks = task, isCurrent = (player == UnitName("player")) })
			end
		end
	end
	...
	if next(auctionItems) then
		...
		tinsert(sources,
			{ sourceName = L["Auction House"], isCrafter = false, isVendor = false, isAH = true, tasks = auctionTask })
	end
	...
	if next(destroyingTask) then
		tinsert(sources,
			{ sourceName = L["Destroying"], isCrafter = false, isVendor = false, isAH = true, tasks = destroyingTask })
	end
